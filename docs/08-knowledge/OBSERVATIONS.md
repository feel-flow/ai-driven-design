...
### OBS-015: 品質ゲートをバックグラウンド実行中にその対象ファイルを編集すると、実行結果は編集前のツリーのものになり無効になる

| Kind | problem | Count | 1 |
| First | 2026-08-28 | Last | 2026-08-28 |
| Status | mitigated | Issue | doc:docs/AI_GIT_WORKFLOW.md#ステップ4-テスト検証test |

`pnpm check` / `npm run quality:local` 等のゲートをバックグラウンドで回している間にソースを編集すると、完了した実行は編集前のツリーを検査した結果でありコミット前ゲートとして使えない → ゲート実行中は対象ファイルを編集せず、待ち時間には対象外の作業（docs 読み・ADR 起案・既存 Issue 検索）だけを充てる。編集したらその実行を無効と見なして測り直す（レビューシムが HEAD 移動で明示的に abort する OBS-011 と異なり、ゲートは無言で古い結果を返す）。
