...
`ACE-XXX` の `XXX` は **PRスコープ式 ID** に置換する: `ACE-<PR番号>-<連番>`（例 `ACE-438-1`、非PR由来は `ACE-i<Issue番号>-<連番>` 例 `ACE-i425-1`）。採番は対象 PR の既存 `ACE-<PR番号>-*` の最大連番 +1（無ければ `-1`）で、全体の最新 ID を読む必要がない（並行採番でも衝突しない）。anchor は ID を小文字化した `<a id="ace-438-1"></a>` を見出し直前に付与する。
