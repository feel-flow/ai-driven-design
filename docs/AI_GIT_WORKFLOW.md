...
**バックグラウンド実行時の注意**: 品質ゲートをバックグラウンドで回している間は、そのゲートの対象ファイル（ソース・テスト・設定）を編集しない。ゲートは開始時にツリーをスナップショットせず各ステージが逐次ファイルを読むため、途中で編集すると旧内容と新内容が混在し、結果が最終ツリーと一致する保証を無言で失う。待ち時間に充てるのは**読み取りだけ**の作業（docs 読み・既存 Issue 検索）に限る。ADR や修正案の下書きは、`docs/**` がゲート対象（Prettier / markdownlint）のため、ゲート対象外のスクラッチ領域で行い、完了後に反映する。対象ファイルを編集した場合はその実行を無効と見なして測り直す（観測: [OBS-018](./08-knowledge/OBSERVATIONS.md#obs-018)）。
...
#### レビュー深度の目安（Risk-Based Workflow）

すべての変更に同じ深さのレビューを課すと、軽微な変更で手続きが形骸化する。変更の規模・種別からレビュー深度を 3 段階で判定する：

```bash
bash scripts/review-level.sh --base develop
# スクリプト連携用（レベル番号のみ）: --quiet
```

| レベル        | 条件                                                                    | レビュー構成                                                         |
| ------------- | ----------------------------------------------------------------------- | -------------------------------------------------------------------- |
| **1（軽量）** | ドキュメントのみ かつ 合計 ≤ 50 行                                      | Toolkit セルフチェックのみ（Codex 省略可）。ACE は知見があるときのみ |
