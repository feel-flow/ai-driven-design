...
**バックグラウンド実行時の注意**: 品質ゲートをバックグラウンドで回している間は、そのゲートの対象ファイル（ソース・テスト・設定）を編集しない。完了した実行は起動時点のツリーを検査した結果であり、途中で編集すると無言で無効になる。待ち時間に充てるのは**対象外**の作業（docs 読み・ADR 起案・既存 Issue 検索）に限り、対象ファイルを編集した場合はその実行を無効と見なして測り直す（観測: [OBS-015](./08-knowledge/OBSERVATIONS.md#obs-015)）。
