...
`## コード生成ルール` → `### 必須事項` 節（現状 `MASTER.md:212` 付近）の 6 番目の「マジックナンバー禁止」の直後に、7 番目として以下を追加する。

```markdown
7. **配置判断**: 新機能追加時の「どこに書くか」は決定木で判断（詳細は `DECISION_TREE.md` を参照）
```

**採用位置の理由**: `必須事項` 節は「AI がコード生成時に必ず守るべきルール」の箇条書きであり、「どこに配置するか」もコード生成時の判断要素として同じ粒度・同じ文体で並べるのが自然。FALLBACK.md は `エラーハンドリング方針` 節から参照されているが、Decision Tree は配置判断のため `コード生成ルール` 節の方が適切。
