...
- 行数の定義: `wc -l` 準拠（`content` を改行 `\n` で分割した行数。末尾改行の扱いをコメントに明記）。
- exit code の整理:
...
- 行数は副作用のない小関数 `countPlaybookLines(content): number` として切り出し、`main()` から呼ぶ。閾値判定（`count > max`）も `main()` 側に置き、テストは「行数カウントの正確性」と「閾値判定の真偽」を別々に検証できるようにする。
