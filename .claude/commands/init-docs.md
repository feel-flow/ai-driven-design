...
| `07-project-management/ROADMAP.md` | `[プロジェクトの長期的なビジョン]` / `[プロジェクトのミッション]` / 開始日 `[YYYY-MM-DD]` | 概要 / 実行日 |
| 全文書共通 | Frontmatter の `owner` / `created` / `updated`（`YYYY-MM-DD`）、変更履歴表の `[日付]` / `[名前]` | owner / 実行日 |

- **番号を持つ例示の衝突回避**: `ADR-001` のように番号を持つ例示は、実プロジェクトの採番と衝突する。テンプレートの例示は非実在番号（`ADR-00X`）を使っているので、索引・一覧を実態で埋めた後も例示側を実番号へ書き換えないこと。例示に実番号が残っている場合は `ADR-00X` 等の非実在番号へ変更する
