...
> 上のADRテンプレートとこの記述例の番号（`ADR-00X` / `ADR-00Y`）は非実在番号です。ADR一覧を実プロジェクトの採番で埋めた後も、コードブロック内の例示を実番号（`ADR-001` 等）に書き換えないでください。同一ファイル内で番号が衝突します。

```markdown
## ADR-00X: PostgreSQL 17.xの採用
