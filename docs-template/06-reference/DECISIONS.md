...
> **⚠️ このファイルの ADR は「記入例」です — 実プロジェクトの決定ではありません**
>
> ADR-001〜ADR-010 は架空プロジェクトのサンプル、ADR-011 は本テンプレートのソースリポジトリの実記録です。
> **プロジェクトに展開したら、これらは削除するか自プロジェクトの実際の決定に置き換えてください。**
> 残したままにすると、AI ツールが「このプロジェクトはマイクロサービス + PostgreSQL + Kafka で決定済み」と誤読し、
> 実態と無関係な前提でコードを生成します。書式は §決定記録テンプレート（AI最適化版）を参照してください。

## ADR-001: マイクロサービスアーキテクチャの採用

### ステータス

承認済み（サンプル記入例 — このプロジェクトの決定ではない）
...
> 以下は記入例です（ファイル冒頭の注記を参照）。自プロジェクトの決定に置き換えてください。

| ADR番号 | タイトル                                                      | 決定日     | ステータス                   | 決定者               |
| ------- | ------------------------------------------------------------- | ---------- | ---------------------------- | -------------------- |
| ADR-001 | マイクロサービスアーキテクチャ                                | 2024-01-15 | 承認済み（例）               | アーキテクトチーム   |
| ADR-002 | PostgreSQL採用                                                | 2024-01-20 | 承認済み（例）               | テックリード         |
| ADR-003 | TypeScript採用                                                | 2024-01-22 | 承認済み（例）               | 開発チーム           |
| ADR-004 | イベント駆動アーキテクチャ                                    | 2024-02-01 | 承認済み（例）               | アーキテクトチーム   |
| ADR-005 | Kubernetes採用                                                | 2024-02-10 | 承認済み（例）               | インフラチーム       |
| ADR-006 | GraphQL API                                                   | 2024-02-15 | 検討中（例）                 | APIチーム            |
| ADR-007 | Zero Trust Security                                           | 2024-02-20 | 承認済み（例）               | セキュリティチーム   |
| ADR-008 | モノレポ採用                                                  | 2024-03-01 | 承認済み（例）               | 開発チーム           |
| ADR-009 | 認証・認可方針                                                | 2024-03-10 | 承認済み（例）               | セキュリティチーム   |
| ADR-010 | 監視スタック                                                  | 2024-03-15 | 承認済み（例）               | SREチーム            |
| ADR-011 | ドキュメント構造再構築 (ai_spec_driven_development.md v2.0.0) | 2025-10-16 | 承認済み（ソースリポの記録） | Docsガバナンスチーム |
...
承認済み（本テンプレートのソースリポジトリの記録 — 採用先では削除してよい）
